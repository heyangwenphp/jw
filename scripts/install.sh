#!/usr/bin/env bash
# ------------------------------------------------------------------
# Jedi installer for macOS / Linux
#
...
if command -v claude &>/dev/null; then
    ...
    if [ "$CLI_INSTALLED" = true ]; then
        ...
        echo ""
        echo "  可以先安装 Jedi，稍后手动安装 Claude CLI："
        echo ""
...
step "Looking for Jedi installer (platform: $PLATFORM)..."
...
install_macos() {
    ...
    # Priority: 1) package-locked DMG, 2) arch-specific DMG, 3) any Jedi DMG
    local dmg
    ...
    if [ -z "$dmg" ]; then
        warn "No matching Jedi .dmg found in the current directory."
        echo "  Please download it from the releases page and install manually."
        return
    fi

    ok "Found: $(basename "$dmg")"
    step "Installing Jedi to /Applications..."
...
install_linux() {
    ...
    if [ -z "$appimage" ]; then
        warn "No Jedi .AppImage found in the current directory."
        echo "  Please download it from the releases page and install manually."
        return
    fi
