...
function getEnabledProviderModels(serviceProviderDefinitions = [], apiProfiles = []) {
  const definitions = Array.isArray(serviceProviderDefinitions) ? serviceProviderDefinitions : []

  const result = []
  for (const provider of definitions) {
    // Only include if provider is explicitly enabled (default true)
    if (provider.enabled === false) continue
    const models = normalizeModelIds(provider.defaultModels)
    for (const modelId of models) {
      result.push({ modelId, providerId: provider.id })
    }
  }
  return result
}
...
export function buildScheduledTaskModelOptions(context = {}) {
  const { apiProfiles = [], serviceProviderDefinitions = [] } = context
  ...
  const enabledModels = getEnabledProviderModels(serviceProviderDefinitions, apiProfiles)

  const seen = new Set()
  const uniqueModels = enabledModels.filter(({ modelId }) => {
    if (seen.has(modelId)) return false
    seen.add(modelId)
    return true
  })

  return uniqueModels.map(({ modelId, providerId }) => ({
    label: modelId,
    value: modelId,
