    ...
    <div class="import-modal-content">
      ...
      <div v-if="step === 1" class="import-step">
        ...
        <input
          ...
          webkitdirectory
          multiple
          @change="handleWebImportFiles('folder', $event)"
        />
        <input
          ref="zipInputRef"
          class="hidden-file-input"
        ...
        <div v-else-if="validation">
          ...
          <div v-if="validation.valid && validation.skills?.length" class="validation-skills">
            ...
            <div class="target-section">
              ...
              <n-checkbox v-model:checked="form.overwriteExisting" class="overwrite-checkbox">
                {{ t('rightPanel.skills.overwriteExistingSkills') }}
              </n-checkbox>
              <p class="overwrite-hint">{{ t('rightPanel.skills.overwriteExistingSkillsHint') }}</p>
            </div>
          </div>
        </div>
      </div>
...
import { NModal, NButton, NRadio, NRadioGroup, NCheckbox, useMessage } from 'naive-ui'
...
const validateSelectedSource = async (sourceForValidation) => {
  ...
  try {
    const result = await window.electronAPI.validateSkillImport(sourceForValidation)
...
const handleWebImportFiles = async (type, event) => {
  try {
    ...
    if (type === 'folder') {
      ...
      const firstPath = payloadFiles[0]?.relativePath || ''
      label = firstPath.split(/[\\/]/).filter(Boolean)[0] || t('rightPanel.skills.importFromFolder')
      sourcePayload = { type: 'folder', files: payloadFiles }
    } else {
      const file = files[0]
      label = file.name
      sourcePayload = {
...
const handleImport = async () => {
  ...
  try {
    ...
    const result = await window.electronAPI.importSkills({
      ...
      overwriteExisting: form.value.overwriteExisting
    })
