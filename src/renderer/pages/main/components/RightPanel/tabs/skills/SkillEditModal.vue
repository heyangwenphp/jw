...
import { toSkillRawContent } from '@utils/skill-edit-utils'
...
const fieldExists = (fieldName) => {
  ...
  return regex.test(toSkillRawContent(form.value.rawContent))
}
...
const addField = (fieldName) => {
  ...
  const content = toSkillRawContent(form.value.rawContent)
...
const loadSkillContent = async (skill) => {
  if (skill) {
    ...
    try {
      ...
      if (result.success) {
        form.value.rawContent = toSkillRawContent(result.content)
      } else {
...
const handleSave = async () => {
  ...
  try {
    const params = {
      ...
      rawContent: toSkillRawContent(form.value.rawContent),
      projectPath: props.projectPath
    }
