  ...
  <n-modal v-model:show="visible" preset="card" :title="t('market.title')" style="width: 680px; max-width: 90vw;">
    <div class="market-modal-content">
      ...
      <n-tabs v-model:value="activeTab" type="segment" size="small">
        ...
        <!-- <n-tab-pane :name="'mcps'" :tab="t('market.tabs.mcps')">暂时隐藏 MCP 市场 -->
        <!--   <MarketList -->
        <!--     :items="filteredMcps" -->
...
const fetchIndex = async () => {
  ...
  try {
    ...
    if (result.success) {
      // 替换作者名：HydroCoder → JediCoder
      const replaceAuthor = (item) => {
        if (item.author === 'HydroCoder') {
          return { ...item, author: 'JediCoder' }
        }
        return item
      }
      marketData.value = {
        skills: (result.data.skills || []).map(replaceAuthor),
        prompts: (result.data.prompts || []).map(replaceAuthor),
        agents: (result.data.agents || []).map(replaceAuthor),
