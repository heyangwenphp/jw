        ...
        <template v-else>
          ...
          <n-form-item :label="t('rightPanel.skills.exportFormat')">
            <n-radio-group v-model:value="form.format">
              <n-radio v-if="!isWeb" value="folder">{{ t('rightPanel.skills.exportAsFolder') }}</n-radio>
...
const handleExport = async () => {
  ...
  let exportPath = null
  if (!isWeb.value) {
    exportPath = await window.electronAPI.selectFolder()
  ...
  try {
    ...
    if (form.value.scope === 'single') {
      result = await window.electronAPI.exportSkill({
        ...
        exportPath,
        format: isWeb.value ? 'zip' : form.value.format
      })
    } else {
