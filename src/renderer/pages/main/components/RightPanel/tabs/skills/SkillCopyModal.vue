      ...
      <n-form :model="form" label-placement="top">
        ...
        <n-form-item :label="t('rightPanel.skills.newSkillId')">
          <n-input
            ...
            :placeholder="newSkillIdPlaceholder"
...
const canKeepSameId = computed(() => {
  return ['built-in', 'official', 'plugin', 'public'].includes(form.value.fromSource) &&
    form.value.toSource !== form.value.fromSource
})

const copyHint = computed(() => canKeepSameId.value
  ? t('rightPanel.skills.sameNameOverrideHint')
  : t('rightPanel.skills.mustRenameHint'))

const newSkillIdPlaceholder = computed(() => canKeepSameId.value
  ? t('rightPanel.skills.sameNameOverridePlaceholder')
  : t('rightPanel.skills.newSkillIdPlaceholder'))
...
const initForm = (skill, projectPath) => {
  ...
  form.value = {
    ...
    newSkillId: ['built-in', 'official', 'plugin', 'public'].includes(fromSource) ? skill.id : '',
    existsInTarget: false
  }
}
...
const handleCopy = async (overwrite = false) => {
  ...
  if (newSkillId === skillId && !canKeepSameId.value) {
