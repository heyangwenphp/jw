  ...
  <div class="tab-container">
    ...
    <div class="tab-content">
      ...
      <div v-else class="skills-list">
        <!-- 内置 Skills (应用自带，只读) -->
        <div v-if="filteredSkills.builtIn.length > 0" class="skill-group">
          <div class="group-header clickable" @click="toggleGroup('builtIn')">
            ...
            <span class="group-icon"><Icon name="box" :size="14" /></span>
            <span class="group-name">{{ t('rightPanel.skills.builtInSkills') }}</span>
            <span class="group-count">({{ filteredSkills.builtIn.length }})</span>
          </div>
          <div v-if="expandedGroups.includes('builtIn')" class="group-items">
            <div
              v-for="skill in filteredSkills.builtIn"
              :key="skill.fullName"
              class="skill-item"
            ...
            >
              <div class="skill-row">
                ...
                <span class="skill-actions-inline">
                  <button v-if="skill.canCopy !== false" class="icon-btn inline" :title="t('rightPanel.skills.copySkill')" @click.stop="showCopyModal(skill)"><Icon name="copy" :size="14" /></button>
                ...
                >
                  <n-switch
                    ...
                    @update:value="(val) => handleToggleDisabled(skill, !val)"
                  />
                </span>
              </div>
        ...
        <SkillGroup group-key="user" :skills="filteredSkills.user"
          :title="t('rightPanel.skills.userSkills')" icon="user" :editable="true" :toggleable="true" :visibility-toggleable="true"
        ...
        <!-- 公开 Skills -->
        <SkillGroup group-key="public" :skills="filteredSkills.public"
          :title="t('rightPanel.skills.publicSkills')" icon="globe" :editable="false"
          :expanded="expandedGroups.includes('public')" @toggle="toggleGroup('public')"
          @click-skill="handleSkillClick"
          :copy="showCopyModal" :copy-title="t('rightPanel.skills.copySkill')"
          :empty-text="t('rightPanel.skills.noPublicSkills')" />
...
const skills = ref({ official: [], user: [], project: [], public: [], builtIn: [] })
const expandedCategories = ref([])
const expandedGroups = ref(['project', 'user', 'public', 'official', 'builtIn'])
...
// Computed
const totalCount = computed(() => skills.value.official.length + skills.value.user.length + skills.value.project.length + skills.value.public.length + skills.value.builtIn.length)
...
const filteredSkills = computed(() => {
  ...
  return {
    ...
    public: filterSkillList(skills.value.public, kw),
    builtIn: filterSkillList(skills.value.builtIn, kw)
  }
})
...
const handleSkillClick = (skill) => {
  if (skill?.disabled || skill?.canUse === false) {
    message.warning(t('common.operationFailed'))
    return
  }
  emit('send-command', `/${skill.name || skill.fullName}`)
}
...
const setSkillDisabled = (skill, disabled) => {
  ...
  const source = skill.source === 'built-in' ? 'builtIn' : skill.source
  if (!source || !Array.isArray(skills.value[source])) return

  skills.value = {
    ...skills.value,
    [source]: skills.value[source].map(item =>
      item.id === skill.id ? { ...item, disabled } : item
    )
  }
}

const setSkillVisibility = (skill, visibility) => {
  if (!skill?.id) return
  ...
  skills.value = {
    ...skills.value,
    [source]: skills.value[source].map(item =>
      item.id === skill.id ? { ...item, visibility } : item
    )
  }
}
...
const handleToggleDisabled = async (skill, disabled) => {
  if (skill?.canManage === false) return
  const previousDisabled = skill.disabled
  setSkillDisabled(skill, disabled)
  try {
    const result = window.electronAPI.toggleSkillDisabled
      ...
      : await window.electronAPI.toggleComponentDisabled('skill', skill.id, disabled)
    if (result.success) {
      await loadSkills({ silent: true })
    } else {
      setSkillDisabled(skill, previousDisabled)
...
const handleToggleVisibility = async (skill, visibility) => {
  if (skill?.canToggleVisibility === false) return
  const previousVisibility = skill.visibility
  setSkillVisibility(skill, visibility)
  try {
    const result = await window.electronAPI.updateSkillVisibility({
      skillId: skill.id,
      visibility
    })
    if (result.success) {
      ...
      setSkillVisibility(skill, previousVisibility)
      message.error(result.error || t('rightPanel.skills.visibilityUpdateFailed'))
    }
  } catch (err) {
    setSkillVisibility(skill, previousVisibility)
...
const loadSkills = async ({ silent = false } = {}) => {
  ...
  try {
    ...
    if (result && typeof result === 'object' && !Array.isArray(result)) {
      skills.value = { official: result.official || [], user: result.user || [], project: result.project || [], public: result.public || [], builtIn: result.builtIn || [] }
    } else {
      const arr = result || []
      skills.value = {
        ...
        public: arr.filter(s => s.source === 'public'),
        builtIn: arr.filter(s => s.source === 'built-in')
      }
    }
  } catch (err) {
    console.error('Failed to load skills:', err)
    if (!silent) {
      skills.value = { official: [], user: [], project: [], public: [], builtIn: [] }
    }
  } finally {
...
.skill-item.disabled .skill-desc { opacity: 0.5; }
...
.skill-toggle-hitbox { margin-left: auto; flex-shrink: 0; display: inline-flex; align-items: center; }
