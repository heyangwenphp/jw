...
const downloadProjectFile = async (target) => {
  if (!target || target.isDirectory || !rootPath.value) return

  const resolvedPath = await window.electronAPI.resolvePath(rootPath.value, target.relativePath)
  const filePath = typeof resolvedPath === 'string' ? resolvedPath : ''
  if (!filePath) return

  const url = buildFileDownloadUrl({ filePath })
...
const handleMenuAction = async ({ action, target }) => {
  ...
  switch (action) {
    case 'insertPath':
      ...
      break
    case 'download':
      await downloadProjectFile(target)
      break
    case 'newFile':
