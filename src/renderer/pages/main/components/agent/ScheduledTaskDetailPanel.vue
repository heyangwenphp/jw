...
const form = ref({ ...createScheduledTaskFormDefaults(''), apiProfileId: null })
...
const loadData = async () => {
  ...
  loading.value = true
  try {
    const [taskList, config] = await Promise.all([window.electronAPI.listScheduledTasks(), window.electronAPI.getConfig?.() || Promise.resolve(null)])
...
watch([serviceProviderDefinitions, defaultProfileId], () => {
