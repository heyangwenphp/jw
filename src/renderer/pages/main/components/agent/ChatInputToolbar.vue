  ...
  <div class="input-toolbar">
    <div class="toolbar-left" ref="toolbarRootRef">
      <div class="model-selector" @click="toggleModelDropdown">
        <Icon :name="sessionToolbarIcon" :size="14" class="model-icon" />
      ...
      <Transition name="dropdown">
        <div v-if="showDropdown" class="model-dropdown">
          ...
          >
            <span class="model-dot" :style="{ backgroundColor: model.color }"></span>
            <span class="option-name">{{ model.label }}</span>
					<span v-if="model.providerName" class="option-provider">{{ model.providerName }}</span>
            <Icon v-if="modelValue === model.value" name="check" :size="14" class="check-icon" />
          </div>
        </div>
      </Transition>

      <div v-if="false" class="project-agent-quick-access">
        <div
          class="project-agent-trigger"
          :class="{ active: showProjectAgentDropdown }"
          title="能力项目"
          @click="toggleProjectAgentDropdown"
        >
          <Icon name="agent" :size="13" />
        </div>
        <Transition name="dropdown">
          <div v-if="showProjectAgentDropdown" class="project-agent-dropdown">
            <ProjectAgentQuickPanel
              :project-path="projectPath"
              @apply="handleProjectAgentApply"
            />
          </div>
        </Transition>
      </div>

      <div v-if="showCapabilityButton" class="cap-quick-access">
        <div
          class="cap-trigger"
      ...
      <div v-if="false" class="schedule-task-btn" :title="t('agent.scheduleDraftTitle')" @click="emit('schedule')">
      ...
      <div v-if="showImageUpload" class="image-upload-btn" :title="t('agent.uploadImage')" @click="emit('trigger-image-upload')">
        <Icon name="image" :size="13" />
      </div>

      <div v-if="showFileUpload" class="file-upload-btn" :title="t('agent.uploadFile')" @click="emit('trigger-file-upload')">
        <Icon name="paperclip" :size="13" />
      </div>

      <div v-if="false" class="clear-input-btn" :title="t('agent.clearInput')" @click="emit('clear')">
...
import ProjectAgentQuickPanel from './ProjectAgentQuickPanel.vue'
...
const props = defineProps({
  ...
  showCapabilityButton: { type: Boolean, default: true },
  showImageUpload: { type: Boolean, default: true },
  showFileUpload: { type: Boolean, default: true },
...
const showProjectAgentDropdown = ref(false)
const capList = ref([])
...
const showWeixinBtn = computed(() => false)
...
const toggleWeixinDropdown = () => {
  ...
  showProjectAgentDropdown.value = false
...
const modelDisplayName = computed(() => {
  ...
  return selected?.label || normalizeModelValue(props.modelValue) || DEFAULT_MODEL_LABEL
})

const sessionToolbarIcon = computed(() => {
  if (props.sessionType === 'dingtalk') return 'dingtalk'
  if (props.sessionType === 'weixin') return 'weixin'
  if (props.sessionType === 'feishu') return 'feishu'
  return 'robot'
})
...
const toggleProjectAgentDropdown = () => {
  showProjectAgentDropdown.value = !showProjectAgentDropdown.value
  showDropdown.value = false
  showCapDropdown.value = false
  showWeixinDropdown.value = false
}

const handleProjectAgentApply = (refs) => {
  emit('use-capability-refs', refs)
  showProjectAgentDropdown.value = false
}
