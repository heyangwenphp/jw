...
const INTERNAL_AGENT_FILE_NAMES = new Set(['wechat_765.sqlite'])

const filterVisibleAgentEntries = (items = []) => {
  if (!Array.isArray(items)) return []
  return items.filter(item => {
    const normalizedName = String(item?.name || '').trim()
    return !INTERNAL_AGENT_FILE_NAMES.has(normalizedName) &&
...
export function useAgentFiles() {
  ...
  const loadDir = async (relativePath = '') => {
    ...
    try {
      ...
      const dirEntries = filterVisibleAgentEntries(result.entries || [])
  ...
  const searchFiles = (keyword) => {
    ...
    searchTimer = setTimeout(async () => {
      ...
      try {
        ...
        searchResults.value = filterVisibleAgentEntries(result.results || [])
      } catch (err) {
        searchResults.value = []
      } finally {
        searchLoading.value = false
