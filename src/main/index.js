  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...
    },
    {
      ...
      submenu: [
        { role: 'reload' },
        { role: 'forceReload' },
        ...(process.env.NODE_ENV === 'development' || process.env.VITE_DEV_SERVER_URL ? [{ role: 'toggleDevTools' }] : [])
      ]
    },
    {
      label: 'Window',
...
function createWindow() {
  ...
  // 跨平台窗口图标（macOS 由 .icns/Dock 处理，Windows/Linux 需要显式设置）
  const iconFile = process.platform === 'win32' ? 'icon.ico' : 'icon.png'
  const windowIconPath = path.join(__dirname, '../../assets', iconFile)
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    const builtFilePath = path.join(__dirname, '../../dist/pages/main/index.html');
    const legacyFilePath = path.join(__dirname, '../renderer/pages-dist/pages/main/index.html');
...
app.whenReady().then(async () => {
  ...
  // 托盘图标路径（开发模式用绝对路径，避免 app.getAppPath() 不可靠）
  // 使用原图 1024x1024，由 Electron 缩放，避免 shell 工具缩放导致细节丢失
  const trayIconCandidates = [
    path.join(__dirname, '../../assets/icon_output.png'),
    path.join(__dirname, '../../assets/icon.png'),
    path.join(__dirname, '../../assets/tray.png')
  ]
  const trayIconPath = trayIconCandidates.find(p => fs.existsSync(p)) || null
  ...
  // 开发模式下设置 Dock 图标（生产模式由 .icns 自动处理）
  ...
  // 设置 IPC 处理器
  setupIPCHandlers(mainWindow, configManager, agentSessionManager, capabilityManager, updateManager, dingtalkBridge, feishuBridge, scheduledTaskService, weixinNotifyService, weixinBridge);
  ...
  powerMonitor.on('resume', () => {
    ...
    resumeTimer = setTimeout(() => {
      ...
      if (feishuBridge) {
        console.log('[Main] Reconnecting Feishu bridge...')
        feishuBridge.restart().catch(err => {
          console.error('[Main] Feishu reconnect failed:', err.message)
        })
      }
