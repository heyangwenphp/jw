...
const UPDATE_PRIMARY_URL = 'http://jedi.heyangwen.com/updates';
const DEFAULT_KIMI_PROFILE_ID = 'default-kimi-profile';
const DEFAULT_KIMI_MODEL_ID = 'kimi-for-coding';
const DEFAULT_KIMI_BASE_URL = 'https://api.kimi.com/coding/';
const KIMI_BUILTIN_MODELS = ['kimi-for-coding'];
const DEEPSEEK_BUILTIN_MODELS = ['DeepSeek-V4-Pro'];
const ZEELIN_DEFAULT_MODEL_ID = 'DeepSeek-V4-Pro';
const RETIRED_BUILTIN_PROVIDER_IDS = new Set([
  'official',
  'qwen',
  'zhipu',
  'minimax',
...
const STALE_KIMI_MODEL_IDS = new Set();

function normalizeModelValue(value) {
  return typeof value === 'string' ? value.trim() : '';
}

function isStaleKimiModelId(value) {
  return STALE_KIMI_MODEL_IDS.has(normalizeModelValue(value).toLowerCase());
}
...
function isZeelinProfile(profile) {
  const provider = normalizeModelValue(profile?.serviceProvider).toLowerCase();
  const name = normalizeModelValue(profile?.name).toLowerCase();
  const baseUrl = normalizeModelValue(profile?.baseUrl).toLowerCase();
  return provider === 'zeelin' || name === 'zeelin' || baseUrl.includes('zeelin.cn');
}

function normalizeKimiDefaultProfile(config) {
  if (!config || typeof config !== 'object') return config;

  const profiles = Array.isArray(config.apiProfiles) ? config.apiProfiles : [];
  const now = new Date().toISOString();
  const looksLikeKimiProfile = (profile) => {
    const provider = normalizeModelValue(profile?.serviceProvider);
    const name = normalizeModelValue(profile?.name).toLowerCase();
    const baseUrl = normalizeModelValue(profile?.baseUrl).toLowerCase();
    return provider === 'kimi' || name === 'kimi' || baseUrl.includes('api.kimi.com/coding');
  };

  const kimiProfile = profiles.find(profile => profile.serviceProvider === 'kimi')
    || profiles.find(looksLikeKimiProfile);

  if (!kimiProfile) {
    const defaultProfile = {
      id: DEFAULT_KIMI_PROFILE_ID,
      name: 'KIMI',
      authToken: '',
      authType: 'api_key',
      ...
      selectedModelId: DEFAULT_KIMI_MODEL_ID,
    ...
    return {
      ...
      apiProfiles: [
        defaultProfile,
        ...profiles.map(profile => profile?.isDefault ? { ...profile, isDefault: false } : profile)
      ],
      defaultProfileId: defaultProfile.id
    };
  }

  let changed = config.defaultProfileId !== kimiProfile.id;
  const nextProfiles = profiles.map(profile => {
    ...
    if (nextProfile.serviceProvider !== 'kimi') {
      nextProfile.serviceProvider = 'kimi';
      changed = true;
    }
    if (!normalizeModelValue(nextProfile.selectedModelId) || isStaleKimiModelId(nextProfile.selectedModelId)) {
      nextProfile.selectedModelId = DEFAULT_KIMI_MODEL_ID;
      changed = true;
    }
    if (!normalizeModelValue(nextProfile.baseUrl)) {
      nextProfile.baseUrl = DEFAULT_KIMI_BASE_URL;
      changed = true;
    }
    if (!normalizeModelValue(nextProfile.name)) {
      nextProfile.name = 'KIMI';
      changed = true;
    }
    if (nextProfile.isDefault !== true) {
...
function normalizeModelSet(modelIds) {
...
function isSameModelSet(left, right) {
  const normalizedLeft = normalizeModelSet(left);
  const normalizedRight = normalizeModelSet(right);

  return normalizedLeft.length === normalizedRight.length
    && normalizedLeft.every((modelId, index) => modelId === normalizedRight[index]);
}

function normalizeKimiOnlyDefaultProviderModels(config) {
  if (!config || typeof config !== 'object') return config;
  const definitions = Array.isArray(config.serviceProviderDefinitions)
    ? config.serviceProviderDefinitions
    : [];
  const profiles = Array.isArray(config.apiProfiles) ? config.apiProfiles : [];
  const customProviderIds = new Set(
    definitions
      .filter(definition => definition?.isCustom === true)
      .map(definition => normalizeModelValue(definition?.id))
      .filter(Boolean)
  );
  ...
  for (const definition of definitions) {
    const providerId = normalizeModelValue(definition?.id);

    if (RETIRED_BUILTIN_PROVIDER_IDS.has(providerId) && !customProviderIds.has(providerId)) {
      changed = true;
      continue;
    }
    ...
    if (providerId === 'kimi') {
      const nextDefinition = {
        ...definition,
        defaultModels: [...KIMI_BUILTIN_MODELS]
      };
      if (!isSameModelSet(definition?.defaultModels, KIMI_BUILTIN_MODELS)) changed = true;
    ...
    if (providerId === 'deepseek') {
      const nextDefinition = {
        ...definition,
        defaultModels: [...DEEPSEEK_BUILTIN_MODELS]
      };
      if (!isSameModelSet(definition?.defaultModels, DEEPSEEK_BUILTIN_MODELS)) changed = true;
      nextDefinitions.push(nextDefinition);
      continue;
    }
  ...
  return {
    ...config,
    serviceProviderDefinitions: nextDefinitions,
    apiProfiles: nextProfiles
  };
}
...
class ConfigManager {
  ...
  constructor(options = {}) {
    // 配置文件路径（支持测试时注入自定义路径）
    this.userDataPath = options.userDataPath || electronApp?.getPath?.('userData') || path.join(os.homedir(), '.config', 'jedi-web');
    ...
    // 默认配置
    const defaultProfileTimestamp = new Date().toISOString();
    ...
    this.defaultConfig = {
      ...
      apiProfiles: [{
        ...
        lastUsed: defaultProfileTimestamp,
        icon: 'K'
      }],
      defaultProfileId: DEFAULT_KIMI_PROFILE_ID,  // 默认 Profile（启动时推荐使用）
      ...
      // 自动更新主源
      updatePrimaryUrl: UPDATE_PRIMARY_URL,
      ...
      settings: {
        ...
        // Agent 模式配置
        agent: {
          outputBaseDir: '',           // 输出根目录，默认 ~/jedi-web-agent-output/
  ...
  load() {
    try {
      ...
      if (fs.existsSync(this.configPath)) {
        ...
        migratedConfig = normalizeKimiDefaultProfile(migratedConfig);
        migratedConfig = normalizeKimiOnlyDefaultProviderModels(migratedConfig);
        ...
        // 迁移所有旧更新源到新服务器
        if (!config.updatePrimaryUrl || config.updatePrimaryUrl !== UPDATE_PRIMARY_URL) {
          migratedConfig.updatePrimaryUrl = UPDATE_PRIMARY_URL;
          console.log('[ConfigManager] Migrated update primary URL to jedi.heyangwen.com');
          needsSave = true;
        }

        // 清除无效的 GitHub 后备源（未配置 releases）
        if (config.updateGithub?.owner || config.updateGithub?.repo) {
          migratedConfig.updateGithub = { owner: '', repo: '' };
          console.log('[ConfigManager] Cleared GitHub update fallback');
          needsSave = true;
        }
  ...
  resetToDefaults() {
    ...
    this.config._skipInitialSync = true;
    return this.save();
  }
  ...
  getAPIConfig() {
    ...
    if (defaultProfile) {
      return {
        ...
        serviceProvider: defaultProfile.serviceProvider || 'kimi',
  ...
  migrateProfileStructure(config) {
    ...
    const nextProfiles = config.apiProfiles.map(rawProfile => {
      ...
      const needsMigration = profile.category !== undefined ||
                            ...
                            (!normalizeModelValue(profile.selectedModelId) && isZeelinProfile(profile)) ||
      ...
      if (!normalizeModelValue(profile.selectedModelId) && isZeelinProfile(profile)) {
        profile.selectedModelId = ZEELIN_DEFAULT_MODEL_ID;
      }
  ...
  migrateToProfiles(config) {
    ...
    const hasOnlyDefaultKimiProfile = config.apiProfiles?.length === 1
      && config.apiProfiles[0]?.id === DEFAULT_KIMI_PROFILE_ID
      && !config.apiProfiles[0]?.authToken;

    // 如果已经有用户 API Profiles，不需要迁移
    if (config.apiProfiles && config.apiProfiles.length > 0 && !(hasOldConfig && hasOnlyDefaultKimiProfile)) {
      return config;
    }
    ...
    const defaultProfile = {
      ...
      serviceProvider: 'kimi',
      description: '',
      baseUrl: oldApi.baseUrl || DEFAULT_KIMI_BASE_URL,
      selectedModelId: normalizeModelValue(oldApi.model) || DEFAULT_KIMI_MODEL_ID,
      requestTimeout: TIMEOUTS.API_REQUEST,
      disableNonessentialTraffic: true,
  ...
  async testAPIConnectionViaHTTP(apiConfig) {
    ...
    return new Promise((resolve) => {
      ...
      try {
        // 1. 构造完整 URL
        let baseUrl = apiConfig.baseUrl || DEFAULT_KIMI_BASE_URL;
  ...
  _getBundledUndiciPath() {
    const appPath = electronApp?.getAppPath?.() || path.resolve(__dirname, '../..');
  ...
  async ensureProxySupport(proxyUrl) {
    ...
    try {
      ...
      // 生成 preload 脚本
      const scriptContent = `// Auto-generated by jedi for MCP proxy support
