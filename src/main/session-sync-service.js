...
class SessionSyncService {
  ...
  async sync() {
    ...
    // Try to ensure database is available
    if (!this.db || (typeof this.db.ensureDb === 'function' && !this.db.ensureDb())) {
    ...
    try {
      ...
      for (const entry of projectDirs) {
        ...
        try {
          ...
          // Ensure database is available for this project
          if (!this.db.ensureDb()) {
            console.log('[Sync] Skipping project (database not available):', projectName)
            continue
          }
    ...
    for (const file of sessionFiles) {
      ...
      try {
        ...
        if (existingSession) {
          ...
          if (!session) {
            console.log('[Sync] Skipping session (database not available):', sessionUuid)
            continue
          }
  ...
  async clearInvalidSessions() {
    ...
    if (!this.db || !this.db.db) {
      return { status: 'error', message: 'Database not available', ...stats }
    }
