...
class SessionDatabaseBase {
  ...
  /**
   * 迁移旧数据库（jedi 从 cc-desktop 升级）
   */
  _migrateLegacyDb(userDataPath) {
    // 旧数据库路径（cc-desktop）
    const legacyPath = path.join(userDataPath, '..', 'cc-desktop', 'sessions.db')
    const legacyDir = path.dirname(legacyPath)

    // 如果旧数据库存在但新数据库不存在，执行迁移
    if (fs.existsSync(legacyPath) && !fs.existsSync(this.dbPath)) {
      console.log('[SessionDB] Migrating legacy database from:', legacyPath)
      try {
        // 确保新目录存在
        ...
        console.log('[SessionDB] Legacy database migrated successfully')
      } catch (err) {
        console.error('[SessionDB] Failed to migrate legacy database:', err.message)
      }
    }
  }
  ...
  ensureDb() {
    ...
    try {
      console.log('[SessionDB] Reconnecting to database...')
      this.init()
      return true
    } catch (err) {
      console.error('[SessionDB] Failed to reconnect:', err.message)
      return false
    }
  }
  ...
  init() {
    ...
    const userDataPath = this._userDataPath || electronApp?.getPath?.('userData') || path.join(os.homedir(), '.config', 'jedi-web')
    this.dbPath = path.join(userDataPath, 'sessions.db')

    // 迁移旧数据库（jedi 从 cc-desktop 升级）
    this._migrateLegacyDb(userDataPath)
  ...
  runMigrations() {
    ...
    const agentConvNewColumns = [
      ...
      { name: 'report_mode', type: 'TEXT' },
      { name: 'report_file_path', type: 'TEXT' }
    ]
    ...
    // agent_messages 表迁移：添加 thinking 字段
    const agentMsgInfo = this.db.prepare("PRAGMA table_info(agent_messages)").all()
    const agentMsgColumns = agentMsgInfo.map(col => col.name)

    if (!agentMsgColumns.includes('thinking')) {
      console.log('[SessionDB] Adding column: agent_messages.thinking')
      this.db.exec("ALTER TABLE agent_messages ADD COLUMN thinking TEXT")
    }

    const projectLibraryItemInfo = this.db.prepare("PRAGMA table_info(project_library_items)").all()
    const projectLibraryItemColumns = projectLibraryItemInfo.map(col => col.name)

    if (!projectLibraryItemColumns.includes('agent_session_id')) {
      console.log('[SessionDB] Adding column: project_library_items.agent_session_id')
      this.db.exec('ALTER TABLE project_library_items ADD COLUMN agent_session_id TEXT')
    }
    const projectLibraryItemNewColumns = [
      { name: 'file_path', type: 'TEXT' },
      { name: 'mime_type', type: "TEXT DEFAULT ''" },
      { name: 'size_bytes', type: 'INTEGER DEFAULT 0' },
      { name: 'original_name', type: "TEXT DEFAULT ''" }
    ]
    for (const col of projectLibraryItemNewColumns) {
      if (!projectLibraryItemColumns.includes(col.name)) {
        console.log(`[SessionDB] Adding column: project_library_items.${col.name}`)
        this.db.exec(`ALTER TABLE project_library_items ADD COLUMN ${col.name} ${col.type}`)
      }
    }
    ...
    const projectWorkspaceInfo = this.db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='project_library_workspaces'").get()
    const needsProjectWorkspaceRebuild = projectWorkspaceInfo?.sql?.includes('REFERENCES project_master_records')

    if (needsProjectWorkspaceRebuild) {
      console.log('[SessionDB] Migrating: rebuilding project_library_workspaces table (project master soft reference)')
      this.db.pragma('foreign_keys = OFF')
      this.db.exec('BEGIN TRANSACTION')
      try {
        this.db.exec(`
          CREATE TABLE project_library_workspaces_new (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        this.db.exec(`
          INSERT INTO project_library_workspaces_new (
            id, master_record_id, name, type, description, created_by_user_id, agent_session_id, created_at, updated_at
          )
          SELECT id, master_record_id, name, type, description, created_by_user_id, agent_session_id, created_at, updated_at
          FROM project_library_workspaces
        `)

        this.db.exec('DROP TABLE project_library_workspaces')
        this.db.exec('ALTER TABLE project_library_workspaces_new RENAME TO project_library_workspaces')
        this.db.exec('COMMIT')
        console.log('[SessionDB] Migration completed: project_library_workspaces table rebuilt')
      } catch (err) {
        this.db.exec('ROLLBACK')
        console.error('[SessionDB] Migration failed: project_library_workspaces rebuild failed', err)
      } finally {
        this.db.pragma('foreign_keys = ON')
      }
    }
  ...
  createTables() {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS project_library_workspaces (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        master_record_id INTEGER,
        name TEXT NOT NULL,
        type TEXT NOT NULL DEFAULT 'custom',
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS project_library_items (
        ...
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (workspace_id) REFERENCES project_library_workspaces(id) ON DELETE CASCADE,
        FOREIGN KEY (parent_id) REFERENCES project_library_items(id) ON DELETE CASCADE
      )
    `)
  ...
  createIndexes() {
    if (!this.ensureDb()) {
      console.log('[SessionDB] Database not available, skipping index creation')
      return
    }
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_project_workspace_user ON project_library_workspaces(created_by_user_id);
      CREATE INDEX IF NOT EXISTS idx_project_workspace_agent_session ON project_library_workspaces(agent_session_id);
      CREATE INDEX IF NOT EXISTS idx_project_items_workspace ON project_library_items(workspace_id);
      CREATE INDEX IF NOT EXISTS idx_project_items_parent ON project_library_items(parent_id);
      CREATE INDEX IF NOT EXISTS idx_project_items_agent_session ON project_library_items(agent_session_id);
...
const SessionDatabase = withUserOperations(
  withProjectLibraryOperations(
    withPromptMarketOperations(
      withScheduledTaskOperations(
        withAgentOperations(
