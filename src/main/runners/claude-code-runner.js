  ...
  async createQuery(messageQueue, options, sessionRef) {
    ...
    const queryOptions = {
      ...
      spawnClaudeCodeProcess: (spawnOpts) => {
        ...
        console.log('[ClaudeCodeRunner] CLI spawned:', {
          command: spawnOpts.command,
          firstArg: spawnOpts.args[0]?.substring(0, 80),
          settingSources: queryOptions.settingSources,
          envBaseUrl: childEnv.ANTHROPIC_BASE_URL || childEnv.ANTHROPIC_API_URL || null,
          envModel: childEnv.ANTHROPIC_MODEL || null,
          hasApiKey: Boolean(childEnv.ANTHROPIC_API_KEY),
          hasAuthToken: Boolean(childEnv.ANTHROPIC_AUTH_TOKEN)
        })
    ...
    if (options.tools) queryOptions.tools = options.tools
