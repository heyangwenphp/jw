...
const HIDDEN_DIRS = new Set([
  // 版本控制 & 工具配置
  '.git', '.claude', '.codex', '.svn', '.hg', '.qoder', '.serena',
  // Node.js / 前端
...
// App-generated runtime files that should never be shown in user-facing file lists.
const INTERNAL_AGENT_FILES = new Set(['wechat_765.sqlite'])
const INTERNAL_AGENT_DIRS = new Set(['.claude', '.codex'])

function isInternalAgentFileName(fileName) {
  const normalizedName = String(fileName || '').trim()
  return INTERNAL_AGENT_DIRS.has(normalizedName) ||
    INTERNAL_AGENT_FILES.has(normalizedName) ||
    normalizedName.toLowerCase().endsWith('.sqlite')
}
...
const TEXT_EXTS = new Set([
  ...
  '.lock', '.prisma', '.proto',
  '.com', '.cn', '.dockerfile', '.makefile'
])

// 无扩展名的文本文件名
const TEXT_FILENAMES = new Set([
  'Dockerfile', 'Dockerfile.dev', 'Dockerfile.prod', 'Dockerfile.local',
  'LICENSE', 'LICENSE.md', 'LICENSE.txt', 'NOTICE', 'NOTICE.txt',
  'Makefile', 'Rakefile', 'Gemfile', 'Gemfile.lock',
  'Procfile', '投入品产品信息.txt', '采购订单.txt'
])
...
const AUDIO_MIME_MAP = {
  ...
  '.m4a': 'audio/mp4', '.aac': 'audio/aac', '.flac': 'audio/flac', '.wma': 'audio/x-ms-wma'
}
