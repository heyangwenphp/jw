...
class AgentSession {
  constructor(options) {
    ...
    this.skipNextResume = false     // Model/profile changed while CLI was inactive; next launch must not resume old SDK state
    this.messageCount = 0
    this.totalCostUsd = 0
    this.messages = []              // 消息历史存储（内存）
    this.pendingThinkingText = ''    // Streamed thinking fallback until the final assistant message is persisted
    ...
    this.reportFilePath = options.reportFilePath || options.meta?.reportFilePath || null
  ...
  toJSON() {
    return {
      ...
      source: this.source,
      taskId: this.taskId,
      reportMode: this.reportMode || this.meta?.reportMode || null,
      reportFilePath: this.reportFilePath || this.meta?.reportFilePath || null
    }
  }
}
