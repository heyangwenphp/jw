...
class AgentFileManager {
  ...
  async readFile(sessionId, relativePath) {
    ...
    try {
      ...
      // 音频文件（返回 base64 data URL）
      if (AUDIO_EXTS.has(ext)) {
        if (stat.size > MAX_AUDIO_SIZE) {
          return { ...base, type: 'audio', tooLarge: true, filePath }
        }
        const buf = await fsp.readFile(filePath)
        const content = `data:${AUDIO_MIME_MAP[ext] || 'audio/mpeg'};base64,${buf.toString('base64')}`
        return { ...base, type: 'audio', content, filePath }
      }

      // Word 文档 (.docx) — 前端用 mammoth.js 解析
      if (ext === '.docx') {
        return { ...base, type: 'word', filePath }
      }

      // Office 表格文档 — 前端通过 raw 端点读取并解析
      if (['.xlsx', '.xls'].includes(ext)) {
        return { ...base, type: 'office', filePath }
      }

      // PDF 文件（返回文件路径，前端用 webview/iframe 渲染）
      if (ext === '.pdf') {
        return { ...base, type: 'pdf', filePath }
      }
