...
class DingTalkBridge {
  ...
  async _createNewSession(staffId, nickname, conversationId, conversationTitle, mapKey, { cwd } = {}) {
    ...
    const session = this.agentSessionManager.create({
      ...
      meta: { conversationId },
      ownerUserId: this.configManager.getConfig()?.dingtalk?.ownerUserId || null
    })
  ...
  async _getAccessToken() {
    ...
    if (!response.ok || !result.accessToken) {
      const detail = result.message || result.msg || result.errorMessage || result.code || response.status
