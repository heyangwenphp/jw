...
class CapabilityManager {
  ...
  async fetchCapabilities(projectPath) {
    try {
      const capabilities = this._discoverLocalCapabilities(new Set(), projectPath)
  ...
  _discoverLocalCapabilities(remoteIds, projectPath = null) {
    const result = []
    try {
      // 扫描本地 skills 目录
      const seenSkillIds = new Set()
      const appendSkillsFromDir = (skillsDir) => {
        ...
        for (const entry of dirs) {
          ...
          const skillMdLower = path.join(skillsDir, id, 'skill.md')
          if (fs.existsSync(skillMd) || fs.existsSync(skillMdLower)) {
            const mdPath = fs.existsSync(skillMd) ? skillMd : skillMdLower
            const meta = this._parseSkillMeta(mdPath)
            result.push({
              id,
              name: meta.name || id,
      ...
      if (projectPath && typeof projectPath === 'string') {
        appendSkillsFromDir(path.join(projectPath, '.codex', 'skills'))
        appendSkillsFromDir(path.join(projectPath, '.claude', 'skills'))
      }
      appendSkillsFromDir(this.skillsDir)
      // 扫描本地 agents 目录
      const seenAgentIds = new Set()
      const appendAgentsFromDir = (agentsDir, source = 'user') => {
        ...
        for (const entry of files) {
          ...
          const meta = this._parseSkillMeta(agentPath)
          result.push({
            id,
            name: meta.name || id,
            description: meta.description || '',
      ...
      if (projectPath && typeof projectPath === 'string') {
        appendAgentsFromDir(path.join(projectPath, '.claude', 'agents'), 'project')
      }
      appendAgentsFromDir(this.agentsDir, 'user')
      // 扫描本地 plugins
      if (fs.existsSync(this.installedPluginsPath)) {
        try {
          const pluginsData = JSON.parse(fs.readFileSync(this.installedPluginsPath, 'utf-8'))
          const plugins = pluginsData.plugins || []
          const enabledPlugins = this._getEnabledPlugins()
          for (const plugin of plugins) {
            const id = plugin.name + '@' + (plugin.marketplace || 'unknown')
            if (remoteIds.has(id)) continue
            const disabled = enabledPlugins ? !enabledPlugins.includes(id) : plugin.disabled
            result.push({
              id,
              name: plugin.name,
          ...
          console.warn('[CapabilityManager] Plugin scan error:', err.message)
        }
      }
    } catch (err) {
      console.warn('[CapabilityManager] _discoverLocalCapabilities error:', err.message)
    }
    return result
  }
  ...
  _parseSkillMeta(mdPath) {
    try {
      ...
      if (!description) {
        const paraMatch = content.match(/^[a-zA-Z].+$/m)
        if (paraMatch) description = paraMatch[0].trim().substring(0, 100)
      }
