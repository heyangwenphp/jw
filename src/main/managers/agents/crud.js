...
const agentsCrudMixin = {
  ...
  async getBuiltInAgents() {
    ...
    for (const agentsDir of this.builtInAgentDirs || []) {
      const agents = this.scanMarkdownFiles(agentsDir)
      for (const agent of agents) {
  ...
  async getAllAgents(projectPath = null) {
    ...
    const existingIds = new Set([...project, ...user, ...plugin].map(agent => agent.id))
    const builtIn = (await this.getBuiltInAgents())
    ...
    return {
      ...
      plugin,
      // 兼容旧接口: 返回扁平列表
      all: [...project, ...user, ...plugin, ...builtIn]
    }
  },
  ...
  async getAgentRawContent(params) {
    try {
      ...
      let agentPath
      if ((source === 'plugin' || source === 'built-in') && providedPath) {
  ...
  async copyAgent(params) {
    try {
      ...
      // 获取源文件路径
      let sourcePath
      if ((fromSource === 'plugin' || fromSource === 'built-in') && fromPath) {
        sourcePath = fromPath
      } else {
