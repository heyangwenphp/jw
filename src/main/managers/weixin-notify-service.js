...
const PRE_CAPTURE_WELCOME_TEXT = '您已经绑定JediDesktop，可以随时接收和发送信息给后台大模型。'
...
class WeixinNotifyService {
  ...
  async _pollAccountOnce(account, pollTimeoutMs) {
    ...
    for (const message of response.msgs || []) {
      ...
      const target = this._upsertTarget({
        ...
        lastSeenAt: Date.now(),
        ...(message.context_token ? { contextExpiredAt: null, lastError: null } : {})
      })
  ...
  async _sendMessageItems({ accountId, targetId, items, text = '', sessionId, imageCount = 0, target: resolvedTarget, account: resolvedAccount } = {}) {
    ...
    if (target.contextExpiredAt) {
      throw new Error('微信接口 sendmessage 失败：目标上下文已失效，请让该微信用户重新给 bot 发送一条消息，然后重新捕获一次。')
    }
