...
class UpdateManager {
  ...
  async checkForUpdates(silent = false) {
    ...
    const config = this.configManager?.getConfig?.()
    const hasFallback = !!(config?.updatePrimaryUrl && config?.updateGithub?.owner && config?.updateGithub?.repo)
  ...
  macOSManualInstall() {
    ...
    try {
      ...
      const scriptPath = path.join(os.tmpdir(), `jedi-install-${Date.now()}.sh`)
